...
def create_parser():
    parser = argparse.ArgumentParser(description='PyTorch GTZAN Training')
    parser.add_argument('--dataset', metavar='DATASET', default='gtzan',
                        choices=datasets.__all__,
                        help='dataset: ' +
                            ' | '.join(datasets.__all__) +
                            ' (default: gtzan)')
    parser.add_argument('--model', default='cifarcnn', type=str, help='the model architecture to use')
    ...
    parser.add_argument('--labeled-batch-size', default=20, type=int,
